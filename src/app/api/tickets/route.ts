    ...
    if (action === 'close' && ticketId) {
      ...
      if (decoded.role === 'admin') {
        if (ticket.status !== 'confirmed by oem' && ticket.status !== 'resolved' && ticket.status !== 'closed') {
          return NextResponse.json(
            { error: 'Ticket must be at least in "confirmed by oem" stage to close or mark out of scope' },
            { status: 400 }
          );
        }
          ...
          data: {
            ...
            attachments: attachmentPath
              ? ticket.attachments
                ...
                : attachmentPath
              : ticket.attachments,
            status: ticket.clientClosed && !out_of_scope ? 'closed' : ticket.status,
            updated_at: new Date(),
            closed_at: ticket.clientClosed && !out_of_scope ? new Date() : ticket.closed_at,
          },
        });
      ...
      if (decoded.role === 'client' || decoded.role === 'clientMember') {
        if (ticket.out_of_scope) {
          return NextResponse.json(
            { error: 'Cannot close ticket: Ticket is marked out of scope and can only be closed by admin' },
            { status: 400 }
          );
        }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { status, timeline, ticket_type } = await req.json();
