...
const EMPLOYEES_BY_CLIENT: Record<string, { id: string; name: string }[]> = {
  ...
  URJAA: [
    ...
    { id: "URJAA-emp2", name: "emp2" },
    { id: "URJAA-emp3", name: "emp3" },
  ],
};
...
const GroupManagementDialog: React.FC<{
  ...
  const [openClientEmployees, setOpenClientEmployees] = useState<string | null>(
    null
  );
  ...
  function toggleEmployeeForClient(clientId: string, empId: string) {
    setClientMembers((prev) => {
      ...
      if (list.has(empId)) list.delete(empId);
      else list.add(empId);
  ...
  const handleSubmit = async () => {
    ...
    let members = Object.values(clientMembers).flat();

    // If no employees picked but client is selected → default to all employees of that client
    if (members.length === 0) {
      selectedClients.forEach((c) => {
        members = [
          ...members,
          ...(EMPLOYEES_BY_CLIENT[c] || []).map((e) => e.id),
        ];
      });
    }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {mode === "create"
              ? "Create a new group by selecting clients and employees."
              : "Update the group details or members."}
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="flex justify-between">
          {mode === "edit" && groupData?.id && (
            <Button
              type="button"
              variant="destructive"
              onClick={() => onDeleteGroup?.(groupData.id!)}
              disabled={isLoading}
            >
              Delete Group
            </Button>
...
const ChatInterface: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `/api/chats?userId=${encodeURIComponent(user.id)}`
        );
    ...
    socket.on("message", (msg: Message) => {
        ...
        prev.map((c) =>
          c.id === msg.chatId
            ? {
                ...
                unreadCount:
                  selectedChatId === msg.chatId || isOwn
                    ? c.unreadCount || 0
                    : (c.unreadCount || 0) + 1,
              }
            : c
      ...
      if (!isOwn) {
        toast({
          ...
          description: msg.text || "Attachment",
        });
      ...
      setChats((prev) =>
        prev.map((c) => ({
          ...
          isOnline: c.isGroup
            ? c.isOnline
            : c.members.some(
                (m) => m !== currentUserId && !!(online && m === userId)
              ),
        }))
      );
    });
    ...
    socket.on("chat_deleted", ({ id }: { id: string }) => {
      ...
      if (selectedChatId === id) {
        ...
        toast({
          title: "Group deleted",
          description: "This group was removed.",
        });
      }
    });
  ...
  const handleCreateGroup = async (data: {
    ...
    try {
      const res = await fetch("/api/chats", {
        ...
        body: JSON.stringify({
          name: data.name,
          memberIds: [currentUserId, ...data.memberIds],
  ...
  const handleRemoveMember = async () => {
    ...
    try {
      const res = await fetch(
        `/api/chats/${selectedChatId}/members/${encodeURIComponent(
          userIdToRemove
        )}`,
        {
          method: "DELETE",
  ...
  return (
    <div className="h-full flex bg-background">
      ...
      <div
        ...
        } border-r flex flex-col transition-[width] duration-150 overflow-hidden`}
      >
        <div className="p-3 border-b flex items-center justify-between gap-2">
          {!sidebarCollapsed ? (
            <h2 className="font-semibold text-lg">Messages</h2>
          ) : (
            <div className="text-sm font-medium">Msgs</div>
          )}
              ...
              >
                <div className="relative">
                  ...
                  {conversation.isOnline && (
                    <div className="absolute -bottom-0.5 -right-0.5 w-2 h-2 bg-green-500 rounded-full border-2 border-background"></div>
                  )}
                </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {conversation.unreadCount &&
                        conversation.unreadCount > 0 && (
                          <Badge
                            variant="destructive"
                            className="ml-2 text-xs h-5 w-5 flex items-center justify-center p-0"
                          >
                            {conversation.unreadCount}
                          </Badge>
                        )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {typingByChat[selectedChatId]?.length
                        ? `${typingByChat[selectedChatId][0]} is typing…`
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {chats.find((c) => c.id === selectedChatId)?.isGroup && (
                      <>
                        ...
                        <DropdownMenuItem
                          onClick={() => handleDeleteGroup(selectedChatId)}
                        >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <button
                          ...
                          onClick={() =>
                            handleReaction(message.chatId, message.id, "👍")
                          }
                        >
                          👍
                        </button>
      ...
      <GroupManagementDialog
        ...
        onSubmit={
          groupDialogMode === "create" ? handleCreateGroup : handleUpdateGroup
        }
