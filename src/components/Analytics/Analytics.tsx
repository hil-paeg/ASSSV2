...
const AdminAnalytics: React.FC = () => {
  ...
  const [tickets, setTickets] = useState<Ticket[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [visibleClients, setVisibleClients] = useState<{ [key: string]: boolean }>({})
      ...
      try {
        ...
        const fetchedTickets = response.data.map((ticket: any) => ({
          ...
          close_ticket: ticket.close_ticket
            ...
            : undefined,
          client: {
            client_username: ticket.client?.client_username || "Unknown",
          },
        }))
        console.log("Fetched tickets:", fetchedTickets)
        setTickets(fetchedTickets)

        // Initialize visibleClients state based on unique clients
        const clients: string[] = [...new Set(fetchedTickets.map((ticket) => ticket.client.client_username))]
        const initialVisibleClients: { [key: string]: boolean } = clients.reduce(
          (acc: { [key: string]: boolean }, client: string) => {
            acc[client] = true
            return acc
          },
          {}
        )
        setVisibleClients(initialVisibleClients)
      } catch (error) {
        console.error("Error fetching tickets:", error)
        alert("Failed to fetch tickets.")
        setTickets([])
  ...
  // Prepare data for star ratings of last 5 tickets per client
  const clients: string[] = [...new Set(filteredTickets.map((ticket) => ticket.client.client_username))]
  const clientRatings = clients.reduce((acc, client) => {
    const clientTickets = filteredTickets
      .filter((ticket) => ticket.client.client_username === client && ticket.status === "closed" && ticket.close_ticket?.rating != null)
      .sort((a, b) => new Date(b.closed_at!).getTime() - new Date(a.closed_at!).getTime())
      .slice(0, 5)
    acc[client] = clientTickets.map((ticket) => ticket.close_ticket?.rating || 0)
    return acc
  }, {} as { [key: string]: number[] })

  // Structure starRatingData for LineChart
  const starRatingData = Array.from({ length: 5 }, (_, index) => {
    const dataPoint: { ticket: string; [key: string]: string | number } = { ticket: `Ticket ${index + 1}` }
    clients.forEach((client) => {
      dataPoint[client] = clientRatings[client][index] || 0
    })
    return dataPoint
  })
  ...
  // Assign colors to clients dynamically
  const clientColorMap = clients.reduce((acc, client, index) => {
    acc[client] = CLIENT_COLORS[index % CLIENT_COLORS.length]
    return acc
  ...
  const handleLegendClick = (client: string) => {
  ...
  return (
    <div className="container mx-auto p-6 space-y-8 bg-gradient-to-br from-slate-50 to-blue-50/30 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="bg-gradient-to-br from-amber-50 to-orange-100 border border-orange-200 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-amber-900 font-medium">Overall Rating</p>
              ...
              <Star className="h-12 w-12 text-amber-800 bg-white/20 p-3 rounded-full" />
            </div>
          </CardContent>
        </Card>

        <Card className="bg-gradient-to-br from-emerald-50 to-green-100 border border-green-200 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-emerald-700 font-medium">Avg Response Time</p>
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-purple-700 font-medium">Total Tickets</p>
                <p className="text-3xl text-purple-600 font-bold mt-1">{totalTickets}</p>
              </div>
              <TicketIcon className="h-12 w-12 text-purple-600 bg-white/20 p-3 rounded-full" />
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="bg-white/80 backdrop-blur-sm shadow-xl hover:shadow-2xl transition-all duration-300 rounded-2xl">
          <CardHeader className="pb-4">
            <CardTitle className="text-gray-800 text-xl font-bold flex items-center gap-2">
              ...
              Star Ratings for Last 5 Tickets per Client
            </CardTitle>
          </CardHeader>
          <CardContent>
            {starRatingData.some((data) => clients.some((client) => data[client] > 0)) ? (
              <ResponsiveContainer width="100%" height={320}>
                <LineChart data={starRatingData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" opacity={0.5} />
                  <XAxis
                    dataKey="ticket"
                    ...
                    formatter={(value) => (
                      <span
                        className={`text-gray-700 font-medium cursor-pointer ${
                          visibleClients[value] ? "" : "opacity-50"
                        }`}
                  ...
                  {clients.map((client) => (
                    visibleClients[client] && (
                      <Line
                        ...
                        dataKey={client}
                        name={client}
                        stroke={clientColorMap[client]}
                        strokeWidth={3}
                        dot={{ r: 6, stroke: clientColorMap[client], strokeWidth: 2, fill: "#fff" }}
                        activeDot={{ r: 8, stroke: clientColorMap[client], strokeWidth: 2, fill: "#fff" }}
                        animationBegin={0}
                        animationDuration={800}
                        animationEasing="ease-out"
                      />
